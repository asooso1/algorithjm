...
N = int(input())
input_case = input()
input_case = list(map(int,input_case.split(' ')))       # input 받은 string을 ' '기준으로 자르고, map()으로 int형으로 변환하고 list화
test_case = sorted(input_case)                          # 오름차순 정렬
print(test_case[int((len(test_case)/2))])               # len()/2 -1번째 (정가운데) 출력